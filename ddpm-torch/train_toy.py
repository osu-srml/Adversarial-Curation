...
def load_generated_data(data_dir, batch_size):
    """
    Loads the generated npy file from the specified directory as a dataset
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use-dataset", action="store_true", help="whether to use the specified dataset")
    parser.add_argument("--data-dir", default="./generated_data/gaussian8", type=str, help="data dir")
    ...
    if args.use_dataset:
        ...
        trainloader = load_generated_data(args.data_dir, args.batch_size)

        data_size = len(trainloader.dataset)
        num_batches = data_size // batch_size

    else:
        print(f"Default：{args.dataset}")
        
        dataset = args.dataset
        data_size = args.size
        num_batches = data_size // batch_size
        dataloader = DataStreamer(dataset, batch_size=batch_size, num_batches=num_batches)
    ...
    chkpt_path = os.path.join(chkpt_dir, f"ddpm_checkpoint.pt")
    print (f"INTRAIN path {chkpt_path}")
    ...
    print (f"Finish INTRAIN path {chkpt_path}")