...
class FilteredCIFAR10Dataset(Dataset):
    """
    Custom dataset for filtered CIFAR10 images.
    """

    def __init__(self, root=None, transform=None):
        if root is None or not os.path.isdir(root):
            raise ValueError(f"No valid images found in {root}. Ensure it contains valid image files.")
        self.root = root
        self.transform = transform or transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
        ])
        self.image_paths = [os.path.join(root, f) for f in os.listdir(root) if f.lower().endswith(('png', 'jpg', 'jpeg'))]
        self.labels = [self._extract_label(f) for f in os.listdir(root) if f.lower().endswith(('png', 'jpg', 'jpeg'))]
    ...
    def _extract_label(self, filename):
        """
        Extract label from filename. Assumes the format 'label_index.extension'.
        """
        try:
            label = filename.split('_')[0]  # Extract the first part before '_'
            return label
        except IndexError:
            raise ValueError(f"Filename '{filename}' does not follow the required format 'label_index.extension'.")
...
@register_dataset
class FilteredCIFAR10(FilteredCIFAR10Dataset):
    """
    Filtered CIFAR10 dataset to mimic torchvision's CIFAR10.
    """
    resolution = (32, 32)
    channels = 3
    transform = transforms.Compose([  # Add class-level default transform
        transforms.RandomHorizontalFlip(),
        transforms.ToTensor(),
        transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
    ])
    ...
    def __init__(self, root, split="train", transform=None):
        """
        Initialize the filtered CIFAR10 dataset.
        ...
        split : str
            Data split ('train' or 'test'). Currently ignored since filtered data has no explicit split.
        transform : callable, optional
            Transformations to apply to the images.
        """
        if split not in ["train", "test"]:
            raise ValueError(f"Invalid split '{split}'. Allowed values are 'train' or 'test'.")
        super().__init__(root=root, transform=transform or self.transform)  # Use class-level transform as default
        self.split = split
